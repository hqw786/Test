...
public class ImageAndTextObject_Modify<T> : MonoBehaviour where T : Graphic
{
    ...
    /// 单向透明度过渡
    /// </summary>
    /// <param name="hide">GameObject默认隐藏</param>
    protected void AlphaTransitionOneWay(bool hide = true)
    {
    ...
    //单向从指定透明度1到到指定透明度2
    public void SetAlphaOneWay(float min, float max, float transTime)
    {
        isAlpha = true;
        minAlpha = min;
        maxAlpha = max;
