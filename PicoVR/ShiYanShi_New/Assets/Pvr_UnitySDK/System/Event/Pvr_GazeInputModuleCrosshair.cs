...
public class Pvr_GazeInputModuleCrosshair : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if (DisplayCrosshair)
		{
            // Set the crosshair distance close to where the user is looking将交叉瞄准器距离靠近用户的视点
	...
	void SetCrossHairAtDistance(float dist)
	{
        ...
        // But keep the crosshair the same perceptable看得见的 size no matter 物质；事件；有关系；要紧 the distance
		Crosshair.localScale = CrosshairOriginalScale * dist*0.3f;
	}
}
