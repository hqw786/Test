...
public class Pvr_ExtraSensor : MonoBehaviour
{
        ...
        if (SensorStart)
        {
            ...
            try
            {
                ...
                if (returns == 0)
                {
					...
					UnityQutation =  new Quaternion(-x-0.7071068f, -y, z, w + 0.7071068f);
                    enable = true;
    ...
    public bool GetUnitySDKPSensorAcceleration()
    {
        bool enable = false;
        try
        {
        ...
        {
            Debug.LogError("GetUnitySDKPSensorAcceleration ERROR! " + e.Message);
            throw;
        }
        return enable;
    }

    public bool GetUnitySDKPSensorAngularVelocity()
    {
        bool enable = false;
        try
        {
        ...
        {
            Debug.LogError("GetUnitySDKPSensorAngularVelocity ERROR! " + e.Message);
            throw;
        }
        return enable;
    }
    ...
    void Update()
    {
        if (SensorCanUse)
        {
            if (GetUnitySDKSensorState())
            {
                ...
                this.gameObject.transform.rotation = UnityQutation;
            }
        }
    }
